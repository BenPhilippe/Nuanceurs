Shader "Test/ToonWaterTest" {
	...
	SubShader {
		...
		Pass{
			...
			float4 _MainTex_ST;
			float _Frequency, _Amount, _Height, _Foam;


			v2f vert(appdata v){
				v2f o;
				float4 tex = tex2Dlod(_NoiseTex, float4(v.uv.xy, 0, 0));
				v.vertex.y += sin(_Time.z * _Frequency + (v.vertex.x * v.vertex.z * _Amount * tex)) * _Height;
